...
let createRect = (x,y, width, heigh, color) =>{
...
let update = () => {
    ...
    while (allRains.length < maximumRainCount && maximumRainInitializationInOneFrame > rainInitCountInOneFrame) {
    ...
    for(let i = 0; i < allRains.length; i++) {
        allRains[i].move(); 
        if(allRains[i].posY > canvas.heigh || allRains[i].posX > canvas.width) {
            allRains.splice(i, 1);
        }
    }
